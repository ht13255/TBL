"""Coherent time-bin transfer matrices for recirculating fiber loops."""
...
class CoherentTimeBinLoop:
    ...
    def transfer_matrix(self, *, seed: int | None = None) -> np.ndarray:
        ...
        for input_bin in range(self.time_bins):
            external = np.zeros(self.time_bins, dtype=complex)
            external[input_bin] = 1.0
            output, _ = self._propagate_with_traces(external, reflectivities, phases)
            transfer[:, input_bin] = output
        singular = np.linalg.svd(transfer, compute_uv=False)
        if singular.size and singular.max() > 1 + 1e-9:
            raise ValidationError("coherent loop construction violated passivity")
        return transfer
